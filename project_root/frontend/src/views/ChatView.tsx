...
export function ChatView() {
  ...
  async function handleMobileConversationChange(value: string) {
    if (value.startsWith('group:')) {
      selectConversation(Number(value.slice(6)))
      return
    }
    if (value.startsWith('member:')) {
      await openMemberChat(value.slice(7))
    }
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="hidden w-72 shrink-0 flex-col border-r bg-muted/10 md:flex">
        ...
        <section className="flex min-w-0 flex-1 flex-col">
          <div className="border-b px-3 py-3 md:px-4">
            ...
            <select
              className="mb-1 flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-base md:hidden"
              value={mobileConversationValue}
              onChange={(event) => void handleMobileConversationChange(event.target.value)}
              aria-label="Conversation"
            >
              {groupConversation ? (
                <option value={`group:${groupConversation.id}`}>{groupConversation.title}</option>
              ) : null}
              {sortedMembers.map((member) => (
            ...
            <h2 className="hidden text-sm font-semibold md:block">{activeTitle}</h2>
            {activeConversation?.kind === 'group' ? (
              <p className="hidden text-xs text-muted-foreground md:block">Workspace group chat</p>
            ) : activeConversation ? (
              <p className="hidden text-xs text-muted-foreground md:block">Direct message</p>
            ) : null}
          </div>
